...
OFFICIAL_TXT_SRC = ['CLMADtrad.txt']
    ...
    with open(src_file, 'r') as fp:
        for line in fp:
            ...
            if idx == line.split(' ')[0]:
                ...
                return int(trans_text), file

    return None, file


    
class VAGDataset(Dataset):
    def __init__(self, path, split, bucket_size, ascending=False):
        # Setup
        print("[VAGDataset] path: {}, split: {}".format(path, split))
        self.path = path
        ...
        for s in split:
            ...
            print("Mozillacv11Dataset {} found wav data: {}".format(s, len(split_list)))
        ...
        self.file_list, self.text = zip(*[(f_name, txt)
                                          for f_name, txt in sorted(zip(new_file_list, text), reverse=not ascending, key=lambda x: len(str(x[1])))])
...
class Mozillacv11TextDataset(Dataset):
