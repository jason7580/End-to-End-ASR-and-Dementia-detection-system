...
class BaseSolver(object):
    ...
    def __init__(self, config, paras, mode):
        ...
        self.device = torch.device(
            'cuda:0') if self.paras.gpu and torch.cuda.is_available() else torch.device('cpu')
        self.amp = paras.amp
        ...
        if mode == 'train':
            ...
            self.verbose('Evaluating result of tr. config @ {}'.format(
                config['core']['config']))
            self.verbose('Evaluating result of tr. ckpt @ {}'.format(
                config['core']['ckpt']))
...
#                 self.paras.load, map_location={'cuda:1': 'cuda:0'})
